...
def deploy_qpus(n_qpus, cores_per_qpu, mem_per_qpu, simulator = "Aer"):
    family = qraise(n_qpus, "03:00:00", n_nodes=2, simulator=simulator, classical_comm=True, co_located = True, cores = cores_per_qpu, mem_per_qpu = mem_per_qpu, partition="lusi2")
    return family
...
def iqpe_benchmarking(angles_list, n_qpus_list, shots, cores_per_qpu, mem_per_qpu, seed):
    for angle in angles_list:
        for n_qpus in n_qpus_list:
            ...
            with open(f"/lusitania_homes/CenitS/jesus.cerrudo/module-cunqa/cunqa/examples/python/cc_examples/results_iterative_QPE/iQPE_results.txt", "a") as f:
                f.write(str_data)
